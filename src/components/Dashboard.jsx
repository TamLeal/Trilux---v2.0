...
const generateRandomProjectData = () => {
  ...
  const progressData = [
    { month: 'Jan', real: Math.floor(Math.random() * 20), planned: Math.floor(Math.random() * 25) },
    { month: 'Fev', real: Math.floor(Math.random() * 30), planned: Math.floor(Math.random() * 35) },
    { month: 'Mar', real: Math.floor(Math.random() * 50), planned: Math.floor(Math.random() * 55) },
    { month: 'Abr', real: Math.floor(Math.random() * 70), planned: Math.floor(Math.random() * 75) },
    { month: 'Mai', real: Math.floor(Math.random() * 90), planned: Math.floor(Math.random() * 95) },
  ];

  const costData = [
    { name: 'Material', value: Math.floor(budget * 0.4), description: 'Insumos e materiais de construção' },
    { name: 'Mão de obra', value: Math.floor(budget * 0.3), description: 'Equipe própria e terceirizada' },
    { name: 'Equipamentos', value: Math.floor(budget * 0.2), description: 'Locação e manutenção' },
    { name: 'Serviços', value: Math.floor(budget * 0.1), description: 'Serviços especializados' }
  ];
...
export default function Dashboard() {
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
          ...
          <CardContent>
            ...
            <p className="text-xs opacity-75 mt-1">{projects.length} em fase inicial</p>
          </CardContent>
        </Card>
  ...
  const renderProject = (project) => {
    ...
    const { status, financial, team, schedule, progressData, costData } = projectData;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {status.progress < status.planned && (
          <AttentionCard
            ...
            description={`Atraso de ${status.planned - status.progress}% no progresso`}
            severity="warning"
          />
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
            ...
            <CardContent>
              ...
              <Progress value={(financial.spent / financial.budget) * 100} className="mt-2 bg-green-400" />
              <p className="text-sm mt-2">
                R$ {financial.spent.toLocaleString()} / R$ {financial.budget.toLocaleString()}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-orange-500 to-orange-600 text-white">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                {schedule.delay < 0 ? '+' : ''}{Math.abs(schedule.delay)} dias
              </div>
              <p className="text-sm mt-2">
  ...
  return (
    <>
      ...
      {activeTab !== 'overview' && projects.map((project) => (
        project.id === activeTab && renderProject(project)
      ))}
    </>
  );
}
